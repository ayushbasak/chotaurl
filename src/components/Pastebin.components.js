...
const Pastebin = ()=>{
    ...
    const getPastebin = async ()=>{
        if(readbinLink.length > 0){
            const { data } = await axios.get(readbinLink)
                                .catch(err => {
                                    console.log(err)
                                })
            if(data.errorId === 2)
                ...
                const temp = data.content.split("<br>").join("\n")
                setReadbinTitle(data.title)
    ...
    return (
        ...
        <div className="flex flex-col lg:flex-row jusitify-between items-evenly p-2 h-auto lg:h-screen">
            <div className="flex flex-col w-full justify-center items-center">
                ...
                {
                    recievedURL && 
                        <span className=" shadow-xl bg-yellow-400 p-7 w-80 lg:w-8/12 text-center m-2 font-bold text-sm lg:text-xl text-white rounded flex flex-col lg:flex-row justify-center items-center lg:flex-shrink-0 border-l-8 border-red-400">
                                <a href = { recievedURL } >
                                    { recievedURL }
                                </a>
            ...
            <div className = "flex flex-col w-full justify-center items-center">
                ...
                <input onChange = { handleReadbinLink } className = "shadow-xl w-11/12 lg:w-8/12 px-10 py-5 m-2 text-lg focus:outline-none rounded border-l-8 border-red-400" placeholder = "Pastebin link or id"></input>
